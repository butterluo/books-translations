...
def my_python_tool(file_name:str,
    ...
    if(final_translated_text is not None):
        with open(translated_path + '/final-publishing-version-book-chunks/' + file_name, 'w',encoding='utf-8') as f:
            f.write(second_translated_text)
    
    if(second_translated_text is not None):
        with open(translated_path + '/second-Proofreading-version-book-churnks/' + file_name, 'w',encoding='utf-8') as f:
            f.write(second_translated_text)
    
    if(first_translated_text is not None):
        with open(translated_path + '/first-version-translated-book-chunks/' + file_name, 'w',encoding='utf-8') as f:
